...
import com.logdemo.core.async.AsyncComp;
import com.logdemo.util.IpUtils;
...
public class LogAspect {
    ...
    @Pointcut("@annotation(com.logdemo.core.annotation.LogOperation)")
    ...
    @Value("${log.serverName}")
    private String exclude_class_names;
    ...
    private void getAnnotationText(LogPackage logPackage, Method method){
        ...
        if (logConfiguration != null){
            Class annotationClass = logConfiguration.getAnnotationClass();
            Annotation methodAnnotation = method.getAnnotation(annotationClass);
            if (methodAnnotation != null){
                // 截取注解的内容
                String str = methodAnnotation.toString();
                int begin = str.indexOf("(");
                int end = str.lastIndexOf(")");
                String substring = str.substring(begin + 1, end);
                logPackage.putVariable(LogVariableKey.DESC, substring);
            }
        }
